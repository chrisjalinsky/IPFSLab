...
def ipfs_format(d,l,iface,port1):
    result = []
    if len(l) > 0:
        for item in l:
            try:
                ip_addr = d[item][iface]['ipv4']['address']
                if ip_addr:
                    string1 = ipfs_swarm_format(ip_addr, port1)
                    result.append(string1)
            except:
                pass
    return result

def ipfs_swarm_format(ip="0.0.0.0", port="4001"):
    fmt = "/ip4/" + ip + "/tcp/" + port
    return fmt
...
class FilterModule(object):
    ...
    def filters(self):
        return {
            ...
            ,'ipfs_format' : ipfs_format
            ,'ipfs_swarm_format' : ipfs_swarm_format
